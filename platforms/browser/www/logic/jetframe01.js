...
Jet.setClass=function(obj,className,add)
{
	...
	if(add)
	{
	    if(ocl.search(pat)>=0)return true;
	    else
	    {
	        if(ocl!=="")ocl+=" ";
	        ocl+=className;
...
{
	if(NetSchedule.currentTask.netData.callback)NetSchedule.currentTask.netData.callback(res,NetSchedule.currentTask.netData.lObj);
...
if(method)switch(method.toLowerCase())
{
	...
	this.add=function(param,value,encode,callback)
	{
