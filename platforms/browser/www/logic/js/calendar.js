...
this.failureTimePercent=fpercent?fpercent:0; this.option=option?option:0; this.failureTimePercent=0; this.modified=0;
};
...
Task.save=function(o,sync)
{
	o.modified=Date.now();//Moment.getStamp(Calendar.activeObject.moment);
	if(!LU.net)LU.net=new NetworkTransfer();
	var n=new NetData();
	//n.clear();
	n.lObj=o;
	n.method="POST";
	n.url=LU.serverRoot+"client/Task/save/";
	n.add("xml",Task.toXml(o),true);
	if(sync){
		n.onerror=function(e){
			//Task.IDB.syncingList.push(this.lObj);//save to back in database
			//Task.IDB.insync();
		};
		n.callback=function(res){
			var id=parseInt(res);
			if(id>0){
				Task.IDB.syncedBack(this.lObj);
				Task.loadList[this.lObj.id].id=id;
				this.lObj.id=id;
				Task.IDB.save(this.lObj);
			}else{
				//Task.IDB.syncingList.push(this.lObj);
				//Task.IDB.insync();
			}
		};
		
	}else{
		Task.IDB.callback=LU.globalCallback?LU.globalCallback:null;
		n.callback=Task.IDB.saved;
		n.onerror=Task.IDB.saveError;
	}
	_ajaxQ.add(n);
	_ajaxQ.run();
};
Task.search=function(uid,param,start,end,dl,ul)
{
	...
	var n=LU.net;
	n.clear();
	n.target=LU.serverRoot+"client/Task/search/";
	...
	n.onerror=Task.IDB.offlineLoad;
	try{
	n.send();
	}
	catch(e){
...
Task.remove=function(o)
{
	...
	var n=LU.net;
	n.clear();
	n.target=LU.serverRoot+"client/Task/remove/";
	if(typeof(o)=="object"){
	    n.add("id",o.id);
	    if(o.dialog){
...
Task.parseXml=function(arg)
{
	...
	else
	{
		...
		tmp=new Task(arg.getAttribute("uid"),arg.getAttribute("title"),arg.getAttribute("color"),start,end,fstart,fend,
...
Task.config=function()
{
	...
	Jet.App.form.Task={};
	Jet.App.form.Task[1]="<table class=\"infobox\"><tr><td>%title-lbl%</td><td><input type=\"text\" id=\"titleTxb\" value=\"%title%\"/></td></tr><tr><td>%index-lbl%</td><td><input type=\"number\" id=\"indexTxb\" value=\"%index%\"/></td></tr><tr><td>%color-lbl%</td><td><input type=\"color\" id=\"colorTxb\" value=\"%color%\"/></td></tr><tr><td>%explain-lbl%</td><td><textarea id=\"explainTxb\">%explain%</textarea></td></tr><tr><td>%repeat-lbl%</td><td id=\"repeatPan\"></td></tr><tr><td>%failurePercent-lbl%</td><td><input type=\"number\" id=\"ftTxb\" value=\"%failurTimePercent%\"/></td></tr></table><div class=\"timeBox\"><div><div class=\"timeLabel\"><div id=\"startLbl\">%start-lbl%</div><div id=\"startPan\"></div></div><div class=\"timeLabel\"><div id=\"endLbl\">%end-lbl%</div><div id=\"endPan\"></div></div></div><div class=\"failurePan\"><div class=\"timeLabel\"><div id=\"fstartLbl\">%failureStart-lbl%</div><div id=\"fstartPan\"></div></div><div class=\"timeLabel\"><div id=\"fendLbl\">%failureEnd-lbl%</div><div id=\"fendPan\"></div></div></div></div><hr style=\"clear:both\"/><center><button onclick=\"Task.onSave()\" class=\"green btn b5\">%save-lbl%</button></center>";
...
Task.loadBack=function(res)
{
	...
	if(LU.offline){
		Task.IDB.offlineLoad();
		if(tasks.length>0)Task.IDB.sync(tasks);
	} else {
		Task.showLoadList();
	}
	Task.IDB.sync(tasks);
...
Task.gms=function(o)
{
	var end = Moment.compare(o.end,o.failureEnd)>0? o.end: o.failureEnd;
	o.monthStamp=end.year+"-"+end.month;
};
Task.showLoadList=function()
{
	var keys=Object.keys(Task.loadList);
	for(var i=0;i<keys.length;i++){
	    Task.setToDays(Task.loadingCalendar,Task.loadList[keys[i]]);
	}
};
...
Task.IDB=function()
{
	...
	this.callback=null;
	this.allowInsync=false;
};


Task.IDB.cursorIndex=0;
Task.IDB.syncingList=null;


Task.IDB.save=function(o,draft)
{
	var dlg=o.dialog;
	o.dialog=null;
	Task.gms(o);
	if(draft){
		LU.db.transaction(['task_draft'],'readwrite').objectStore('task_draft').add(o);
	} else {
		LU.db.transaction(['task'],'readwrite').objectStore('task').add(o);
	}
	o.dialog=dlg;
};
Task.IDB.sync=function(ls)
{
	if(ls){
		//update dbase
		Task.IDB.syncingList=ls;
		Task.IDB.cursorIndex=0;
		Task.IDB.syncFetch();
	} else {
		//save changes to server
		Task.IDB.syncingList=[];
		/*LU.db.transaction(["task_draft"],"readonly").openCursor().onsuccess=function(e){
			var cursor=e.target.result;
			if(cursor){
			Task.IDB.syncingList.push(cursor.value);
			}
			else{
				Task.IDB.syncBack();
			}
...
Task.IDB.saveError=function()
{
	Task.IDB.save(Task.activeObject,true); //save to drafts
};
Task.IDB.saved=function(res)
{
	Task.activeObject.id=parseInt(res);
	var draft=Task.activeObject.id>0?false:true;
	Task.IDB.save(Task.activeObject,draft);
	if(Task.IDB.callback)Task.IDB.callback(res);
};
Task.IDB.syncFetch=function()
{
	if(Task.IDB.syncingList.length>Task.IDB.cursorIndex){
		var o=Task.IDB.syncingList[Task.IDB.cursorIndex];
		Task.gms(Task.IDB.syncingList[Task.IDB.cursorIndex]);
		var req=LU.db.transaction(["task"],"readonly").objectStore("task").get(o.id);
		req.onsuccess=function(e){
			if(Task.IDB.syncingList[Task.IDB.cursorIndex].modified>e.target.result.modified){
				var req2=LU.db.transaction(["task"],"readwrite").objectStore("task").put(Task.IDB.syncingList[Task.IDB.cursorIndex]);
				req2.onsuccess=function(e){
					Task.IDB.syncFetch();
				};
			}
			else{
				//add to ajaxQ to set on server
			}
		};
		req.onerror=function(e){
			var req2=LU.db.transaction(["task"],"readwrite").objectStore('task').add(Task.IDB.syncingList[Task.IDB.cursorIndex]);
			req2.onsuccess=function(e){
				Task.IDB.syncFetch();
			};
		};
		Task.IDB.cursorIndex++;
	}
	else{
		Task.IDB.sync();
	}
};
Task.IDB.syncBack=function()
{
	LU.db.transaction(['task_draft'],'readonly').objectStore('task_draft').openCursor().onsuccess=function(e){
		var cursor=e.target.result;
		if(cursor){
			cursor.value.id=-(cursor.primaryKey);
			Task.save(cursor.value,true);
			cursor.continue();
		} else {
			//LU.db.transaction(['task_draft'],'readwrite').objectStore('task_draft').clear();
			//Task.IDB.allowInsync=true;
			//Task.IDB.insync();
		}
	};
};
Task.IDB.offlineLoad=function(arg)
{
	//load both from drafts and synced list
	var ending=LU.db.transaction(['task'],'readonly').objectStore('task').index('ending');
	
	ending.openCursor(/*IDBKeyRange.lowerBound(Calendar.activeObject.getMS())*/).onsuccess=function(e){
			var cursor=e.target.result;
			if(cursor){
				Object.setPrototypeOf(cursor.value,Task.prototype);
				Task.loadList[cursor.value.id]=cursor.value;
				cursor.continue();
			} else {
				var ending_draft=LU.db.transaction(['task_draft'],'readonly').objectStore('task_draft').index('ending');
				ending_draft.openCursor(/*IDBKeyRang.lowerBound(Calendar.activeObject.getMS())*/).onsuccess=function(e){
					var cursor=e.target.result;
					if(cursor){
						Object.setPrototypeOf(cursor.value,Task.prototype);
						if(cursor.value.id>0){
							if(Task.loadList[cursor.value.id]){
								if(Task.loadList[cursor.value.id].modified<cursor.value.modified)
									Task.loadList[cursor.value.id]=cursor.value;
							} else Task.loadList[cursor.value.id]=cursor.value;
						}
						else{
							cursor.value.id=-(cursor.primaryKey);
							Task.loadList[cursor.value.id]=cursor.value;
...
Task.IDB.insync=function()
{
	console.log('depricated insync');
	if(Task.IDB.allowInsync){
		var insynced=Task.IDB.syncingList.pop();
		if(insynced){
			if(insynced.id<0)
			LU.db.transaction(['task_draft'],'readwrite').objectStore('task_draft').add(insynced,Math.abs(insynced.id)).onsuccess=function(e){
				Task.IDB.insync();
			};
		}
		else{
			Task.IDB.allowInsync=false;
		}
	}
};
Task.IDB.syncedBack=function(o)
{
	LU.db.transaction(["task_draft"],"readwrite").objectStore('task_draft').delete(Math.abs(o.id));
};
...
Aniversary.search=function(owner,param,calendar,similar,min,max,dl,ul)
{
	...
	n.callback=LU.globalCallback?LU.globalCallback:null;
	n.onerror=function(e){
		if(this.callback)this.callback("<P>run callback for continue to load calendar</p>");//continue to load
	};
	try{
	n.send();
	}catch(ex){
