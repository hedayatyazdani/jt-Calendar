...
class Task
{
	...
	public static function install()
	{
		...
		$q="create table if not exists $table(id_fl int primary key auto_increment,index_fl int,title_fl varchar(100),color_fl varchar(8),start_fl datetime,end_fl datetime,uid_fl varchar(20),rep_fl int,proj_fl int,fstart_fl datetime,fend_fl datetime,fp_fl double,opt_fl int)";
		$defdb->run($q);
	...
	public static function save( $o)
	{
		...
		if($o->id!=0)
		{
			$q="update $table set index_fl ={$o->index},title_fl ='{$o->title}',color_fl ='{$o->color}',start_fl ='{$o->start}',end_fl ='{$o->end}',rep_fl ={$o->repeat},proj_fl ={$o->project},fstart_fl ='{$o->failureStart}',fend_fl ='{$o->failureEnd}',fp_fl ={$o->failureTimePercent},opt_fl ={$o->option} where id_fl=$o->id and uid_fl='$o->uid'";
			$defdb->run($q);
		}
		else
		{
			$q="insert into $table (index_fl ,title_fl ,color_fl ,start_fl ,end_fl ,uid_fl ,rep_fl ,proj_fl ,fstart_fl ,fend_fl ,fp_fl ,opt_fl )values({$o->index}, '{$o->title}', '{$o->color}', '{$o->start}', '{$o->end}', '{$o->uid}', {$o->repeat}, {$o->project}, '{$o->failureStart}', '{$o->failureEnd}', {$o->failureTimePercent}, {$o->option})";
			$defdb->run($q);
	...
	public static function toXml( $o)
	{
		if(is_array($o))
		{
			...
			return "<Task id=\"$o->id\" index=\"$o->index\" title=\"$o->title\" color=\"$o->color\" start=\"$o->start\" end=\"$o->end\" uid=\"$o->uid\" data=\"$o->data\" repeat=\"$o->repeat\" project=\"$o->project\" failureStart=\"$o->failureStart\" failureEnd=\"$o->failureEnd\" failureTimePercent=\"$o->failureTimePercent\" option=\"$o->option\">$o->explain</Task>";
	}
	public static function parseXml( $arg)
	{
		...
		else
		{
			...
			return new Task($arg->getAttribute("uid"),$arg->getAttribute("title"),$arg->getAttribute("color"),$arg->getAttribute("start"),$arg->getAttribute("end"),
